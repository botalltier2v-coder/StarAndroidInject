...
static void *xmalloc(size_t size) {
    ...
    p = malloc(size);
    if (!p) {
        printf("%s", (char *)OBFUSCATE("Out of memory\n"));
        exit(1);
    }
    return p;
}
...
static int do_load(int fd, symtab_t symtab) {
    ...
    if (0 > rv) {
        LOGD("%s", (char *)OBFUSCATE("read\n"));
        goto out;
    }
    if (rv != sizeof(ehdr)) {
        LOGD("%s", (char *)OBFUSCATE("elf error 1\n"));
        goto out;
    }
    if (strncmp((const char *) ELFMAG, (const char *) ehdr.e_ident, SELFMAG)) { /* sanity */
        LOGD("%s", (char *)OBFUSCATE("not an elf\n"));
        goto out;
    }
    if (sizeof(Elf32_Shdr) != ehdr.e_shentsize) { /* sanity */
        LOGD("%s", (char *)OBFUSCATE("elf error 2\n"));
        goto out;
    }
    ...
    if (rv != size) {
        LOGD((char *)OBFUSCATE("elf error 3 %d %d\n"), rv, size);
        goto out;
    }
    ...
    if (rv != size) {
        LOGD((char *)OBFUSCATE("elf error 4 %d %d\n"), rv, size);
        goto out;
    }
    ...
    for (i = 0, p = shdr; i < ehdr.e_shnum; i++, p++)
        if (SHT_SYMTAB == p->sh_type) {
            if (symh) {
                LOGD("%s", (char *)OBFUSCATE("too many symbol tables\n"));
                goto out;
            }
            symh = p;
            ...
            dynsymh = p;
                   ...
                   && !strncmp(shstrtab + p->sh_name, (char *)OBFUSCATE(".strtab"), 7)) {
            if (strh) {
                LOGD("%s", (char *)OBFUSCATE("too many string tables\n"));
                goto out;
            }
            strh = p;
        } else if (SHT_STRTAB == p->sh_type
                   && !strncmp(shstrtab + p->sh_name, (char *)OBFUSCATE(".dynstr"), 7)) {
            if (dynstrh) {
                LOGD("%s", (char *)OBFUSCATE("too many string tables\n"));
                goto out;
            }
    ...
    if ((!dynsymh && dynstrh) || (dynsymh && !dynstrh)) {
        LOGD("%s", (char *)OBFUSCATE("bad dynamic symbol table\n"));
        goto out;
    }
    if ((!symh && strh) || (symh && !strh)) {
        LOGD("%s", (char *)OBFUSCATE("bad symbol table\n"));
        goto out;
    }
    if (!dynsymh && !symh) {
        LOGD("%s", (char *)OBFUSCATE("no symbol table\n"));
        goto out;
    }
...
static symtab_t load_symtab(char *filename) {
    ...
    if (0 > fd) {
        LOGE((char *)OBFUSCATE("%s open\n"), __func__);
        return NULL;
    }
    if (0 > do_load(fd, symtab)) {
        LOGE((char *)OBFUSCATE("Error ELF parsing %s\n"), filename);
        free(symtab);
        symtab = NULL;
    }
    close(fd);
...
static int load_memmap(pid_t pid, struct mm *mm, int *nmmp) {
    ...
    sprintf(p_buf, (char *)OBFUSCATE("/proc/%d/maps"), pid);
    fd = open(p_buf, O_RDONLY);
    if (0 > fd) {
        LOGE((char *)OBFUSCATE("Can't open %s for reading\n"), p_buf);
        free(p_buf);
        return -1;
    }
    ...
    while (1) {
        ...
        if (0 > rv) {
            LOGE((char *)OBFUSCATE("%s read"), __FUNCTION__);
            free(p_buf);
            return -1;
        }
        if (0 == rv)
        ...
        p += rv;
        if (p - p_buf >= buf_size) {
            LOGE("%s", (char *)OBFUSCATE("Too many memory mapping\n"));
            free(p_buf);
            return -1;
        }
    }
    ...
    while (p) {
        ...
        rv = sscanf(p, (char *)OBFUSCATE("%08lx-%08lx %*s %*s %*s %*s %s\n"), &start, &end, name);
...
static int find_libname(const char *libn, char *name, int len, unsigned long *start,
    ...
    for (i = 0, m = mm; i < nmm; i++, m++) {
        ...
        if (!strncmp((char *)OBFUSCATE("so"), p, 2) || 1) // || (p[0] == '-' && isdigit(p[1])))
            break;
    }
    if (i >= nmm)
        /* not found */
...
int find_name(pid_t pid, const char *name, const char *libn,
    ...
    if (0 > load_memmap(pid, mm, &nmm)) {
        LOGD("%s", (char *)OBFUSCATE("cannot read memory map\n"));
        return -1;
    }
    if (0
        > find_libname((char *) libn, (char *) libc, sizeof(libc),
                       &libcaddr, mm, nmm)) {
        LOGD((char *)OBFUSCATE("cannot find lib: %s\n"), libn);
        return -1;
    }
    //LOGD("lib: >%s<\n", libc)
    s = load_symtab(libc);
    if (!s) {
        LOGD("%s", (char *)OBFUSCATE("cannot read symbol table\n"));
        return -1;
    }
    if (0 > lookup_func_sym(s, (char *) name, addr)) {
        LOGD((char *)OBFUSCATE("cannot find function: %s\n"), name);
        return -1;
    }
    *addr += libcaddr;
    return 0;
...
int find_libbase(pid_t pid, const char *libn, unsigned long *addr) {
    ...
    if (0 > find_libname(libn, libc, sizeof(libc), &libcaddr, mm, nmm)) {
        LOGD("%s", (char *)OBFUSCATE("cannot find lib\n"));
        return -1;
    }
    *addr = libcaddr;
    return 0;
